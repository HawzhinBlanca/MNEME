#!/usr/bin/env bash
# Fast executable contract check for validation-lane --help and -h.
set -euo pipefail
...
if [[ -e "$sentinel_target" ]]; then
  echo "validation-lane-help-smoke: --help created target dir: $sentinel_target" >&2
  exit 1
fi

if [[ -e "$short_sentinel_target" ]]; then
  echo "validation-lane-help-smoke: -h created target dir: $short_sentinel_target" >&2
  exit 1
fi
