...
run_phase_two_three_four_redteam() {
  ...
  run_step "Store bind_external_action (gate-off + phase_iii_bind)" \
    bash -c 'cargo test -p mneme-store --features phase_iii_bind --test phase_iii_bind bind_external_action_fail_closed -- --nocapture && cargo test -p mneme-store --features phase_iii_bind --test phase_iii_bind -- --nocapture'
