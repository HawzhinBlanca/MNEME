...
| **IV** — Scale & standard (research) | 0/4 | 4/4 | 0/4 | **~50%** | PIOP docs + federation verify sketch; no prover/verifier/SDK shipped. **Trick #1 (beacon spot-check):** research doc + Appendix B manifest + crossref selector stub — statistical audit deterrence only, not per-call ZK or global exact-NN on every recall. **Trick #4 (Byzantine inference):** field 8 wire + unanimous verify + `verify-cert --byzantine` + crossref stub — consistency evidence only. |
