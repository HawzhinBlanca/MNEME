...
| CompleteTopK store recall path | Wire `Store::recall_verified` / semantic index to emit complete-kNN proofs from live embeddings (not just cert assembly tests). | Offline `verify-cert` + `complete_knn_cert_v1` round-trip green; `SemanticIndex::recall_receipt_zkann(CompleteTopK)` fails closed. |
| CompleteTopK cross-impl vector | Add Appendix B / `mneme-crossref` wire case for receipt field 8 + level tag 2. | In-repo cert encode/decode only; independent crossref parity not yet committed. |
