...
pub enum MnemeError {
    ...
    IndexPathInvalid,
    #[error(
        "procedure replay mismatch: receipt proves procedure-faithfulness over committed data, not exact nearest-neighbor optimality and not exact top-k under the committed quantized metric (quantized top-k may differ from real-valued top-k). Phase I ExactDominance proves membership/completeness plus top-k over prover-asserted distances; true top-k ranking is not proven and it is not top-k by true query-to-embedding distance until verifiers recompute candidate distances (§3 honesty boundary)"
    )]
    ProcedureMismatch,
    ...
    CertificateInvalid,
    #[error(
        "retrieval dominance proof failed: receipt proves procedure-faithfulness over committed data, not exact nearest-neighbor optimality and not exact top-k under the committed quantized metric (quantized top-k may differ from real-valued top-k). Phase I ExactDominance proves membership/completeness plus top-k over prover-asserted distances; true top-k ranking is not proven and it is not top-k by true query-to-embedding distance until verifiers recompute candidate distances (§3 honesty boundary)"
    )]
...
mod tests {
    ...
    fn assert_distance_caveat(surface: &str, text: &str) {
        for phrase in [
            ...
            "not exact",
            "exact top-k under the committed quantized metric",
            "quantized top-k may differ from real-valued top-k",
