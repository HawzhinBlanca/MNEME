...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            http_addr: "127.0.0.1:0".parse().expect("parse"),
            grpc_addr: None,
...
pub fn test_state(store_path: &Path) -> (AppState, KeyPair, KeyPair) {
    let operator = KeyPair::generate();
    let agent = KeyPair::generate();
    let store = Store::create(store_path, operator.clone()).expect("create store");
    ...
    (state, operator, agent)
}

pub async fn start(config: ServerConfig, store_path: &Path) -> RunningServer {
    let (state, _op, _agent) = test_state(store_path);
    start_with_state(config, state).await
}

pub async fn start_with_state(config: ServerConfig, state: AppState) -> RunningServer {
    ...
    if let Some(addr) = config.grpc_addr {
        let listener = TcpListener::bind(addr).await.expect("bind grpc");
        let bound = listener.local_addr().expect("grpc local");
...
pub fn cap_to_b64(cap: &mneme_cap::Capability) -> String {
    use base64::Engine;
    let bytes = cap.to_bytes().expect("cap canonical");
