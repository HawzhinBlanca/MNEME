...
fn test_optimistic_verifier_and_watcher_challenges() {
    ...
    let t1 = Instant::now();
    for _ in 0..iters {
        SparseMerkleTree::verify_membership(&omitted_proof).unwrap();
    }
    ...
    // Serialize the embedding inline from its public fields (dim ‖ scale ‖ components)
    // so this benchmark does not require adding a method to the frozen mneme-core.
    challenge_bytes.extend_from_slice(&watcher_challenge.counterexample_vector.dim.to_le_bytes());
    challenge_bytes.push(watcher_challenge.counterexample_vector.scale as u8);
    for c in &watcher_challenge.counterexample_vector.components {
        challenge_bytes.extend_from_slice(&c.to_le_bytes());
    }
