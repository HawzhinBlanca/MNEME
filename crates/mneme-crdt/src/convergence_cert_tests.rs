...
use mneme_core::{LogicalKey, ObjectId, PayloadEnc, TrustTier, encode_canonical, from_bytes_strict, hash_obj};
...
fn sample_record(writer: [u8; 32], kind: MemoryKind, wall_ms: u64) -> ObjectRecord {
    ObjectRecord {
        ...
        writer,
        session: [0u8; 16],
        hlc: HlcWire { wall_ms, counter: 0, node_id: [0x02; 16] },
...
fn canonical_dag_root(ids: &[[u8; 32]], objects: &HashMap<[u8; 32], Vec<u8>>) -> [u8; 32] {
    ...
    for id in ids {
        ...
        dag.update_heads(ObjectId(*id), &record.parent_ids).expect("dag");
    }
    dag.root()
}
...
fn merge_convergence_certs_match_after_bidirectional_sync() {
    ...
    let key = LogicalKey { namespace: "user".into(), name: "k1".into() };
    let key_hash = key.hash();
    let bytes_a = record_bytes(&sample_record(writer_hash(&op.public_key_bytes()), MemoryKind::Working, 10));
    let id_a = hash_obj(&bytes_a);
    let bytes_b = record_bytes(&sample_record(writer_hash(&agent_b.public_key_bytes()), MemoryKind::Working, 20));
    ...
    let mut k2o_a = HashMap::from([(key_hash, id_a)]); let mut ok_a = HashMap::from([(id_a, key.clone())]); let mut dag_a = DagIndex::new();
    let peer_b = PeerSnapshot {
        key_index: { let mut ki = SparseMerkleTree::new(); ki.upsert(key_hash, id_b); ki },
        key_to_object: HashMap::from([(key_hash, id_b)]), object_keys: HashMap::from([(id_b, key.clone())]),
        objects: HashMap::from([(id_b, bytes_b.clone())]), dag: DagIndex::new(),
    };
    ...
    let mut k2o_b = HashMap::from([(key_hash, id_b)]); let mut ok_b = HashMap::from([(id_b, key.clone())]); let mut dag_b = DagIndex::new();
    let peer_a = PeerSnapshot {
        key_index: { let mut ki = SparseMerkleTree::new(); ki.upsert(key_hash, id_a); ki },
        key_to_object: HashMap::from([(key_hash, id_a)]), object_keys: HashMap::from([(id_a, key.clone())]),
        objects: HashMap::from([(id_a, bytes_a)]), dag: DagIndex::new(),
    };
