...
pub enum ConvergenceVerify {
    Converged,
    VersionMismatch,
    KeyIndexRootMismatch,
    DagHeadRootMismatch,
    ObjectMsetMismatch,
}
impl ConvergenceVerify {
...
impl ConvergenceCert {
    pub fn build(
        ...
        Self {
            version: CONV_CERT_VERSION,
            key_index_root,
            dag_head_root,
            object_mset_commit: mset.commitment(),
...
pub fn verify_convergence(local: &ConvergenceCert, peer: &ConvergenceCert) -> ConvergenceVerify {
    if local.version != peer.version {
        return ConvergenceVerify::VersionMismatch;
    }
    if local.key_index_root != peer.key_index_root {
        return ConvergenceVerify::KeyIndexRootMismatch;
    }
    if local.dag_head_root != peer.dag_head_root {
        return ConvergenceVerify::DagHeadRootMismatch;
    }
    if local.object_mset_commit != peer.object_mset_commit {
        return ConvergenceVerify::ObjectMsetMismatch;
    }
...
pub fn encode_convergence_cert(cert: &ConvergenceCert) -> Result<Vec<u8>, MnemeError> {
    ...
    encode_text_map(
        ...
        vec![
            (
                "dag_head_root",
                CborValue::Bytes(cert.dag_head_root.to_vec()),
            ),
            (
                "key_index_root",
                CborValue::Bytes(cert.key_index_root.to_vec()),
            ),
            ("object_count", CborValue::Unsigned(cert.object_count)),
            (
                "object_mset_commit",
                CborValue::Bytes(cert.object_mset_commit.to_vec()),
            ),
...
pub fn decode_convergence_cert(bytes: &[u8]) -> Result<ConvergenceCert, MnemeError> {
    ...
    let mut object_mset_commit = None;
...
fn parse_fixed32_value(value: &CborValue) -> Result<[u8; 32], MnemeError> {
    let b = value.as_bytes().ok_or(MnemeError::SchemaDrift)?;
    if b.len() != 32 {
...
mod tests {
    use super::*;
    #[test]
    fn conv_cert_honesty_ceiling_t6() {
    ...
    #[test]
    fn conv_cert_wire_roundtrip() {
        let cert = ConvergenceCert::build([1u8; 32], [2u8; 32], [[3u8; 32], [4u8; 32]]);
        assert_eq!(
            decode_convergence_cert(&encode_convergence_cert(&cert).expect("encode"))
                .expect("decode"),
            cert
        );
    }
    #[test]
    fn conv_cert_verify_detects_mset_mismatch() {
        let roots = ([0x11u8; 32], [0x22u8; 32]);
        assert_eq!(
            verify_convergence(
                &ConvergenceCert::build(roots.0, roots.1, [[0x01u8; 32]]),
                &ConvergenceCert::build(roots.0, roots.1, [[0x02u8; 32]])
            ),
            ConvergenceVerify::ObjectMsetMismatch
        );
    }
    #[test]
    fn conv_cert_verify_converged_same_multiset() {
        let roots = ([0x11u8; 32], [0x22u8; 32]);
        let ids = [[0xaa_u8; 32], [0xbb_u8; 32]];
        assert_eq!(
            verify_convergence(
                &ConvergenceCert::build(roots.0, roots.1, ids),
                &ConvergenceCert::build(roots.0, roots.1, ids.iter().copied().rev())
            ),
            ConvergenceVerify::Converged
        );
    }
