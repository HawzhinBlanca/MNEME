...
fn verify_store_object_filename_rejection_is_named_not_schema_collapsed() {
    ...
    let root_head_load_site =
        source_between_markers(&store, "fn read_head(", "fn load_previous_root(");
    let (_, previous_root_load_site) = store
        .split_once("fn load_previous_root(")
        .expect("load_previous_root marker");
    assert!(
        root_head_load_site.contains("CheckpointLog::read_head(path)")
            && previous_root_load_site.contains("CheckpointLog::try_read_checkpoint(path"),
        "verify_store root loading should delegate to mneme-root no-follow checkpoint readers"
    );
    assert!(
        !root_head_load_site.contains("fs::read(")
            && !previous_root_load_site.contains("fs::read("),
        "verify_store root loading must not use raw path-following fs::read"
    );

    assert!(
        !previous_root_load_site.contains("MnemeError::SchemaDrift"),
